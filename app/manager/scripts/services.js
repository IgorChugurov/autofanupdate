...
.service('$fileUploadNew', ['$http', function ($http) {
        this.uploadFileToUrl = function(file, uploadUrl,id,params){
           // console.log(file);
            var fd = new FormData();
            fd.append('file', file);
...
function addNewSpark($q,$uibModal) {
  return service;
  function service(item,producers){
    return $q(function(resolve,reject){
      var options={
        animation: true,
        templateUrl : 'components/customer/newSpark.html',
        controller: function($uibModalInstance,exception,$resource,Confirm,item,producers){
          var self=this;
          ...
          self.cancel = function () {
            ...
            function tagTransform(name) {
                ...
                Confirm('создать производителя - '+name+'?')
                    ...
                    .then(function (res) {
                        console.log(res);
                        self.item.producer._id = res.id;
                        console.log(self.item.producer);
                        item._id = res.id;
                        //self.item.producer=name;
                    })
                    .catch(function (err) {
                        console.log(err);
        ...
        resolve:{
          item: function(){return item;},
            producers: function(){return producers;}
        }
      };
...
function addNewWork($q,$uibModal) {
    ...
    function service(item){
        return $q(function(resolve,reject){
            var options={
                animation: true,
                templateUrl : 'components/customer/newWork.html',
