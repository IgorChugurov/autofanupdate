...
exports.add= function(req, res,next) {
    if (req.query.update){
        ...
        //let invoice = await JobTicket.findOne({_id:req.body._id}).lean().exec();
        //console.log(invoice)
        JobTicket.update({_id:req.body._id},{$set:newVal}, function (err,result) {
            if (err) return  next(err)
            console.log(result);
        ...
        JobTicket.update({_id: stuff.id}, upsertData, {upsert: true}, function (err) {
            if (err) return next(err);
            // saved!
            res.json({_id: stuff.id,id:stuff.id});
        });
    }
